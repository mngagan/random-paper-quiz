        ...
        <VFX.VFXProvider>
            <div className='container'>
                <div className='col-md-12'>
                    {!showVFX && <span className='lottery' style={{ color: color }}>{randomElement}</span>}
                    {showVFX && <VFX.VFXSpan className='lottery' shader="rainbow">{randomElement}</VFX.VFXSpan>}
                </div>
                {/* <button onClick={() => props.setPage('showNumbers')}>next</button> */}
                <FaHome className='nextIcon' onClick={() => { props.setPage('showNumbers') }} />
            </div>
        </VFX.VFXProvider>
    )
}
